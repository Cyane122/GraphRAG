...
- [정합성] `scene/current.md`는 현재 장면 H2 전체를 단일 patch로 교체하고 활성 인물의 공유 위치·활동을 character 문서에 중복 저장하지 않도록 검증.
- [복구] Wiki Updater 재시도에 직전 JSON 파싱·정책 검증 실패 사유를 전달해 온도 0 모델의 동일 응답 반복을 방지.
...
- [개발 정책] WikiRAG가 GraphRAG의 사용자 기능과 상태 의미에 도달할 때까지 Wiki를 최우선 개발 대상으로 확정. GraphRAG 변경은 치명적 버그, 공용 런타임 회귀와 parity 검증으로 제한.
- [문서] `architecture_wiki/TODO.md`에 사용자 편의, prompt 품질, 핵심 상태, 장기 시뮬레이션 순서의 Graph→Wiki parity 보드와 공동 개발 전환 조건을 추가.
...
- [UI 검증] Hosted UI의 Graph/Wiki 서버 렌더링과 Wiki commit 상태·복구 제어 노출을 실제 빌드 산출물 기준으로 검사하는 테스트를 추가.
- [사용자 편의] Sites Wiki 입력창에 Obsidian 정본 수정이 다음 메시지에서 자동 재로딩된다는 안내와 변경안 상태 새로고침을 추가.
- [진단] 새 Wiki thread 물질화 완료 시 prompt 비가시 `.wikirag-runtime.json` 표식을 기록하고, 표식이 없는 이전 구현 thread를 commit 상태 API와 turn debug에서 `legacy`로 구분.
- [이력] 새 Wiki patch에 계획 시점 `base_markdown`을 보존하고 applied archive의 `applied_changes`에 section별 before/after 원문과 SHA-256을 기록해 inverse patch와 3-way merge 기반을 마련.
- [정책] applied section이 그대로면 즉시 inverse, 이미 before면 멱등 처리, 수동 편집이 있으면 `after → before` 변경을 현재 원문에 3-way 적용하고 겹치는 변경은 사용자 선택으로 중단하는 복구 규칙을 확정.
- [기능] `src/wiki/rollback.py`에 applied commit의 쓰기 없는 inverse 계획과 line-based 3-way merge를 구현. 다른 줄의 수동 편집은 보존하고 같은 줄·section 구조 충돌은 모든 Markdown write 전에 중단한다.
- [이력] Inverse를 원본 archive 수정이 아닌 `operation: inverse`와 `source_commit_id`를 가진 새 audited commit으로 기록하고, 메시지/commit 연결용 user·assistant message ID와 `wiki_commit_id`를 영속화.
- [메시지 안전성] 후속 턴 없는 최신 applied Wiki 응답의 edit/delete/reroll/variant를 inverse 후 허용하고, Actor 재생성 실패 시 inverse의 inverse로 원래 정본을 보상 복구.
- [사용자 편의] Sites 메시지 메뉴에 `Wiki 상태 검토`를 추가해 inverse 가능 여부, before/current 충돌 비교와 명시적 상태 되돌리기를 제공. 충돌 시 현재 Markdown을 유지하며 자동 덮어쓰지 않음.
- [대화 안전성] 적용된 중간 과거 Wiki 턴에서 원본 대화와 Markdown을 직접 수정하는 대신, thread vault를 새 ID로 복사하고 이후 message-linked applied commit을 역순 inverse해 선택 입력 직전 상태를 재구성하는 안전 분기를 추가. 복구 충돌 시 불완전 branch를 제거하고 원본을 유지.
- [사용자 편의] Sites 메시지 메뉴에 `여기서 분기`를 추가해 재구성된 Wiki 대화로 이동하고 선택한 사용자 입력을 작성창 초안으로 복원.
- [사용자 편의] Wiki 대화 이름 변경, 보관/복원, 대화 JSON과 canonical Markdown/commit archive ZIP 내보내기, 확인 후 영구 삭제를 Sites 대화 메뉴에 추가.
- [삭제 안전성] Wiki 대화 삭제 시 검증된 thread vault를 먼저 staging하고 대화 JSON과 함께 제거하며, 중간 실패 시 둘을 복구하도록 해 반쪽 삭제를 방지. ZIP에서는 실행 lock과 updater debug를 제외.
- [프롬프트] Wiki 턴을 Graph와 같은 공용 rule/scene-only LLM 장면 분류 경로에 연결하고, legacy·vulnerable 라벨을 실제 prompt asset key로 정규화.
- [프롬프트 정합성] 분류되어도 아무 규칙도 주입하지 않던 0바이트 `daily`, `bonding`, `formal`, `tense`, `conflict`, `action`, `ambient` 공용 장면 prompt를 장면 목표·물리/상태 연속성·과잉 전개 방지·열린 종결 규칙으로 채워 Graph/Wiki 공용 회귀를 수정.
- [상태 정합성] Wiki accepted Actor 헤더의 시각·장소를 `scene/current.md`에 결정적으로 동기화. 시간 역행을 무시하고 날짜 점프·장소 이동에는 사용자 입력의 명시적 근거를 요구하며, Updater가 scene patch를 생략해도 안전한 변경만 현재 장면 H2 전체 patch로 보류.
- [상태 생성] Wiki Updater에 durable Event 전용 구조화 `CreateDocument`를 추가. exact quote·confidence·ID/경로 중복·Actor 유래 플레이어 행동을 검증한 뒤 template 기반 `events/<slug>.md`를 만들고 source commit과 메시지 쌍을 frontmatter에 연결.
- [복구] 새 Event 문서를 section patch와 함께 지연 적용하고 생성 원문/revision을 archive에 기록. Inverse는 미수정 문서만 삭제하고 수동 편집 시 충돌로 중단하며, inverse의 inverse는 삭제 문서를 원문 그대로 보상 복구.
- [사용자 편의] Sites Wiki 변경안 카드에 새 Event 문서의 경로와 근거를 적용 전에 확인하는 별도 목록을 추가.
- [상태 생성] Wiki `CreateDocument`를 owner-private Memory로 확장. 관련 Event, profile owner, exact evidence source를 검증하고 기억 내용·해석·감정·확신·왜곡 가능성을 canonical `memories/<slug>.md`로 렌더링.
- [권한] Actor 응답은 현재 Actor owner, player 입력은 player owner의 Memory만 만들 수 있게 하고, Memory template의 player visibility를 제거.
- [프롬프트 격리] Actor prompt에는 현재 NPC profile과 owner가 일치하는 Memory만 포함하고 Updater에는 모든 owner 문서를 유지. 관련 Event 내부 ID는 본문에 노출하지 않고 Event 제목으로 컴파일.
...
- [비용] Actor와 Wiki Updater의 추론 강도(thinking level)를 앱 전역 설정으로 노출하고 설정창에서 LOW/MEDIUM/HIGH로 바꿀 수 있게 함. Wiki Updater는 그동안 추론 설정을 넘기지 않아 JSON 응답용 기본값이 대신 적용됐고, 실측에서 생성 토큰의 대부분과 호출 지연 대부분이 추론 단계에서 발생하고 있었다. 이제 호출 시점에 명시적으로 지정하며, 값을 넘기지 않는 호출자는 이전 동작을 그대로 유지한다.
- [경계] 추론 강도는 앱 계층에서 요청 모델의 필드로 주입한다. Wiki 패키지는 앱 설정을 직접 읽지 않으며, 이 값은 프롬프트 조립과 문서 계약에 관여하지 않는다. Gemini 계열이 아닌 Actor 모델에는 적용되지 않는다.
- [개발 인프라] 사용자 인터페이스 소유권을 명문화. 새 화면과 컨트롤은 활성 클라이언트에서 구현하고, 레거시 로컬 클라이언트는 엔진 API 호환 유지와 치명적 수정으로 범위를 제한한다. 두 클라이언트가 같은 엔진 API를 쓰므로 기존 엔진 동작을 노출하기만 하는 변경은 클라이언트 단독 작업으로 본다.
- [문서] WikiRAG 아키텍처 문서의 구현 범위 절이 실행 보드와 어긋나 있던 것을 정리. 이미 끝난 항목(문서 생성, 권한 격리, inverse와 3-way 충돌, 이력·진단·migration, 결정적 recall 등)을 반영하고, 남은 범위는 인덱스·편집 도구·누적 문서 압축·장기 플레이 실측·독립 실행으로 다시 적었다.

## 2026-07-30

- [개발 인프라] 자동 트리거되던 `python-dev-style` 계열(`python-dev-style`, `python-file-header`, `python-refactor-safe`, `python-test-review`)을 제거하고, `plan`/`implement`/`review` 모드의 수동 `/python-dev` 플러그인으로 통합했다.
- [문서] `AGENTS.md`에는 안정적인 Python 저장소 법칙만 남기고, `CLAUDE.md`에는 Claude→Codex Python 오케스트레이션 규칙을 분리해 기록했다.
- [상태 정합성] 턴 추출이 기존 사건·기억 문서를 patch하지 못하도록 차단. 두 종류는 문서 소유 규칙에 각각 "객관적 발생 기록"과 "한 소유자의 주관적 회상"으로 정의돼 있고 기억의 가변 섹션은 게이트형 왜곡 후처리기가 소유하는데, 검증에는 두 종류에 대한 분기가 아예 없어 모델이 과거 기록을 조용히 다시 쓸 수 있었다. 이제 생성 경로만 허용하고 patch는 문서와 사유를 밝히며 거부한다. 후처리기 경로는 검증 이후에 병합되므로 영향받지 않는다.
- [상태 정합성] 한 턴이 서로 다른 참여자·장소·결과를 갖는 복수의 지속 사건을 만들 수 있음을 프롬프트에 명시. 출력 스키마와 검증은 이미 복수 생성을 지원했으나 규칙과 예시가 단일 사건만 다뤄, 한 턴에 별개의 사건이 함께 일어나면 하나로 합쳐지거나 누락될 여지가 있었다. 같은 발생을 두 번 기록하는 것은 계속 금지한다.
- [개발 인프라] compiled prompt 회귀 게이트를 버전 관리되지 않는 저작 원고의 전문 해시에서 구간별 구조 fingerprint로 옮김. 기존 게이트는 저작 편집과 조립 코드 회귀를 구분할 수 없어 원고를 고칠 때마다 빨간불이 됐고 실제로 그 상태로 방치돼 있었다. 이제 블록 이름·순서·개수·구간 배치가 하드 게이트이고 원고 편집은 비치명적 drift 통지로만 보고된다. Fixed/Genre 캐시 안정성 검증은 그대로 유지한다. 검증 대상 목록이 실제 저작 자산과 어긋나 새로 추가된 항목이 어떤 게이트에도 걸리지 않던 문제도 함께 수정.
- [개발 인프라] 매 턴 읽히는 필수 런타임 자산이 광범위 `*.json` 무시 규칙에 걸려 버전 관리에서 빠져 있던 문제를 수정. 새 clone에서는 이 자산이 없어 모든 Wiki 턴이 실패한다. 무시 규칙 전체를 완화하지 않고 해당 자산만 예외로 두며, 전체가 의도적으로 제외된 콘텐츠 디렉터리 안의 파일은 계속 제외한다.
- [상태 정합성] 비밀의 공개 상태를 런타임 소유로 고정. 이 값은 본문에 있어 턴 추출이 자유롭게 바꿀 수 있었는데, 인물이 비밀을 볼 수 있는지는 frontmatter의 인지자 목록만으로 결정되고 그 목록은 생성 시점에만 기록된다. 따라서 상태만 공개로 넘어가면 출력 누설 차단은 풀리는 반면 방금 알게 된 인물의 prompt에는 문서가 계속 들어가지 않는 반쪽 전환이 됐고, 되돌리는 방향도 막는 것이 없어 확립된 공유 지식을 조용히 지울 수 있었다. 이제 상태 값 변경과 누락을 거부하고, 같은 섹션의 공개 단서·오해 갱신은 그대로 허용한다. 기존 인지자 목록에 인물을 추가하는 경로는 커밋 모델에 metadata 연산이 없어 별도 작업으로 남긴다.
- [상태 정합성] Updater patch와 생성 문서의 본문 계약 검사를 프롬프트 조립 시점에서 커밋 전으로 옮김. wikilink나 저장 metadata가 섞인 변경안이 근거 검증만 통과해 정본에 먼저 기록되고, 그 다음 턴 조립에서 예외가 터져 손으로 Markdown을 고칠 때까지 대화가 막히는 순서였다. 이제 같은 계약을 커밋 전에 적용해 정본을 오염시키기 전에 거부한다. 검사 구현은 조립 경로와 같은 것을 재사용하고 복제하지 않는다.
- [경계] 산문 헤더의 시각·장소 파서를 DB 의존 상태 모듈에서 순수 모듈로 분리. Wiki 상태 모듈이 시각 해석만을 위해 그래프 드라이버까지 import하던 결합을 끊었고, 기존 호출부는 재수출로 동작이 그대로다. import 시 그래프 드라이버를 끌어오던 경로 셋 중 하나가 제거됐으며, 남은 둘은 공용 프롬프트 조립과 공용 장면 key 정규화 재사용으로 의도된 설계다.

## 2026-07-31

- [개발 인프라] 스모크 하네스가 검증용 문서에 주입한 frontmatter가 조용히 누락될 수 있던 문제를 수정. 주입 헬퍼가 렌더링 결과와 정확히 일치하는 문자열 치환에 의존했는데, 렌더러는 값을 따옴표로 감싸므로 치환이 한 번도 매치되지 않았다. 주입이 통째로 사라져도 검사는 통과하므로, 실제로는 아무것도 확인하지 못하는 검사가 초록불로 남는다. 이제 주입 위치를 값 표기에 의존하지 않게 잡고 반영 여부를 되읽어 단언한다.
- [상태 정합성] 여러 턴에 걸친 사건을 Event 하나로 담을 수 있게 함. Event 문서는 생성 후 불변이라 진행 중인 활동을 담을 그릇이 없었고, 실제로는 턴마다 별개 Event로 잘리거나 완결 전에 불완전하게 기록됐다. Goal·Item·Secret이 이미 쓰는 "정체성(읽기 전용) + 상태(가변)" 2섹션 패턴을 Event에도 적용해, `## 진행 상태`만 갱신 가능하게 하고 `## 발생 정보`와 `## 사건 내용`은 계속 불변으로 둔다. 원본 발생 기록은 절대 다시 쓰이지 않고 진행 경과만 누적되므로 객관적 로그 성격은 유지된다. 상태 기본값은 종료이므로 한 턴짜리 사건의 동작은 이전과 같고, 다중 턴은 명시적으로 선택해야 한다. 종료된 기록을 다시 여는 전이, 상태 줄 누락, 잘못된 상태 값, 상태 줄 중복은 모두 커밋 전에 거부한다.
- [상태 정합성] 한 턴에 별개 사건이 여럿 있을 때 각각 기록되도록 판별 기준을 명시. 복수 생성을 허용하는 규칙은 있었지만 주변 규칙이 보수적으로 읽혀 실측 Event 42건 중 복수 생성은 0건이었다. 이제 참여자·장소·결과가 다르면 별개 사건이고 한 활동의 연속 단계는 아니라는 기준과 예시를 함께 준다. 같은 발생의 중복 기록 금지는 유지한다.
- [상태 정합성] 생성된 Event마다 같은 응답 안에서 `related_event_id`가 일치하는 Actor 소유 Memory를 요구하도록 바꿈. 프롬프트가 "모든 사건마다 기억을 만들지 말라"고 명시적으로 지시하고 있어, 객관적 사건 기록만 남고 그 사건을 겪은 인물의 주관적 기억이 비는 경우가 실측에서 사건 생성 커밋의 약 3분의 1이었다. 이제 프롬프트 규칙과 커밋 전 검증 게이트가 함께 강제하며, 게이트는 누락된 event ID를 지목해 기존 재시도 루프가 그대로 고치도록 한다. 플레이어 소유 기억은 Player Input에 인용 가능한 근거가 있을 때만 추가로 만들고, 근거가 없으면 만들지 않는다. Actor 서술이 플레이어 기억의 권한이 될 수 없다는 경계는 유지한다.
- [상태 정합성] Player Input이 플레이어 캐릭터의 신체·감정 상태를 명시하면 해당 `현재 상태` 섹션을 갱신하라는 긍정 규칙을 추가. 플레이어 문서에 대해서는 금지 규칙만 있고 갱신을 지시하는 규칙이 없어, 입력에 근거가 있는데도 생략하는 쪽으로 쏠렸다. 실측에서 캐릭터 문서 변경의 근거 출처는 Actor 응답 119건 대 Player Input 18건이었다. Actor 서술이 플레이어 상태의 권한이 될 수 없다는 규칙과 런타임 소유 섹션 경계는 그대로 둔다.
- [개발 인프라] 실제 플레이로 생긴 Wiki 상태 변화를 턴 순서대로 읽을 수 있는 읽기 전용 분석 스크립트 `scripts/wiki_change_log.py`를 추가. 변화 내용은 이미 thread vault의 커밋 archive에 섹션별 before/after 원문과 근거·확신도까지 남고 있었지만, 커밋마다 파일이 따로이고 파일명이 불투명한 ID라 한 번에 훑을 수단이 없어 수동 검사에 의존해야 했다. 이제 커밋 archive와 대화 기록을 대조해 각 변경을 그것을 유발한 턴 옆에 unified diff로 렌더링하고, 재시도·낮은 확신도·건너뜀·외부 편집·되돌리기·같은 섹션 반복 수정을 조사 대상으로 따로 모은다. 런타임 턴 경로에는 계측을 넣지 않았다. 이미 저장되는 자료만 읽으므로 지연 비용이 없고 deferred commit 경로를 건드리지 않는다.

## 2026-08-01

- [프롬프트] Wiki 프롬프트 조립이 선택적 추가문 파일(`cot_append.md`, `blacklist.md`)을 Graph 모드와 동일한 규칙으로 반영하게 함. COT 추가문은 턴 checklist에 들어가고 더 구체적인 추가문이 있으면 그것이 우선하며, blacklist 추가문은 기존 Fixed blacklist의 추가 슬롯에 정확히 한 번 들어간다. 파일이 없을 때의 기존 조립 결과는 유지한다.
...
- [기능] 대화별 산문 프롬프트 variant(A/B/C) 선택을 추가했다. Actor에게 들어가는 pov·emotion·style 프롬프트 모듈 조합을 대화 단위로 바꿀 수 있고, variant A는 기존 조립과 완전히 동일하게 유지해 실험의 대조군 역할을 한다. variant B는 style 블록 뒤에 완화 지침을 이어붙이고, variant C는 pov·emotion·style 세 블록을 하나의 통합 prose 블록으로 대체한다. 체크리스트의 감정 스캔 줄도 선택된 variant에 맞춰 함께 바뀐다.
- [기능] 리롤이 부모 응답과 다른 variant를 선택할 수 있게 하고, 메시지와 각 저장 버전마다 실제로 생성에 쓰인 variant를 기록하게 했다. 이후 실측 비교에서 어떤 응답이 어떤 variant로 만들어졌는지를 응답 자체에서 추적할 수 있다.
- [UI] hosted-ui 작성창에 산문 variant 선택 컨트롤을 추가하고, 메시지 메타 줄에 응답을 생성한 variant 이름을 함께 표시하도록 했다.
- [모델] Actor 모델 목록에 `gemini-3.8-flash`를 추가했다.
- [기능] 산문 prompt variant에 D를 추가했다. A의 지침 내용은 그대로 유지하면서 pov·emotion·style 세 블록에 흩어져 중복돼 있던 규칙만 구조적으로 정리한 버전으로, C처럼 세 블록을 하나의 prose 블록으로 합쳐 내보내되 완화된 지침은 섞지 않아 체크리스트도 A와 동일하게 유지한다.
- [기능] hosted-ui의 두 번째 채팅 라우트(`/notion`)를 기본 라우트와 기능 동등 수준으로 확장했다. 대화별 유저노트 CRUD, OOC 턴 지시 편집, Wiki 실험 시스템 토글·이전 대화 상태 구조 보강·외부 Markdown 편집 감지·문서 무결성 진단·읽기 전용 문서 탐색기, 커밋 재시도·재생성과 상태 되돌리기 검토, 캐릭터 위치 이동과 DB 구조 보기, 대화 이름 변경·ZIP 내보내기·보관·영구 삭제, 엔진 주소·테마·글꼴·포인트 컬러·Output Repair·응답 및 Wiki 갱신 사고 단계를 다루는 설정 패널을 추가했다. 기존 라우트의 스큐어모픽 장면 헤더 대신 쓰던 인라인 러너 방식은 그대로 유지했다.
- [버그픽스] `/notion` 라우트의 좌측 대화함 폭이 모바일 폭에서 데스크톱과 다른 값으로 벌어지던 불일치를 정리했다. 또한 명시적으로 고른 화면 테마가 시스템의 다크 모드 선호에 밀려 배경색이 어긋나 보이던 문제와, 화면을 오버스크롤할 때 테마와 무관한 배경색이 잠깐 드러나던 문제를 함께 고쳤다.
- [기능] 대화 요약 API 응답에 Wiki 캐릭터 표시 이름 필드를 추가했다. 기존 필드는 그대로 유지되는 추가 전용 변경이다.
- [기능] NPC 행동 지침 모듈을 POV 선택과 같은 방식으로 variant별로 우선 로딩하게 했다. variant D는 A/B/C가 쓰는 공용 `core/NPC_BEHAVIOR.md` 대신 자체 축약본을 불러오고, A/B/C는 이전과 동일하게 공용 모듈을 그대로 쓴다.

## 2026-09-12

- [기능] 대화별 엔진 모듈 선택을 추가했다. 산문 variant가 "어떤 문장으로 쓸지"를 고르는 것과 달리, 엔진 모듈은 인과·NPC 자율성·사건 기록·다인 장면 배분 같은 운영 규칙을 슬롯 단위로 얹는다. 슬롯 7개를 각각 끄거나 변형 하나를 고를 수 있고, 선택값은 산문 variant와 같은 자리에 저장돼 캐시되는 Fixed 세그먼트에 실린다. 전부 끈 상태의 조립 결과는 이전과 바이트 단위로 동일해서, 기존 A/B/C/D 대조군이 그대로 유지된다.
- [기능] 메인 엔진과 기억 엔진은 사용자 서술 정책(일반·사칭방지·사칭)을 공유한다. 한쪽에서 정책을 바꾸면 켜져 있는 다른 쪽도 같은 정책으로 따라가고, 꺼진 슬롯은 켜지지 않는다. 이 불변식은 화면이 아니라 서버 정규화에서 강제하므로 API를 직접 호출해도 어긋난 조합이 저장되지 않는다.
- [기능] 리롤과 메시지 수정 경로까지 엔진 선택값을 각인하게 했다. 저장된 이전 버전은 자기가 생성될 때 쓰인 선택값을 그대로 유지하므로, 응답마다 어떤 구성으로 만들어졌는지 응답 자체에서 추적할 수 있다.
- [안전성] 엔진 모듈 본문은 외부에서 가져온 원문이라 중괄호가 많아, 프롬프트 조립의 변수 치환을 그대로 태우면 11개 중 4개가 예외를 던지며 턴 전체를 깨뜨렸다. 이 본문만 치환 대상에서 제외하는 표식을 두고, 이름 토큰만 단순 문자열 치환으로 채우게 했다. 나머지 프롬프트 조각의 치환 동작은 그대로다.
- [UI] 산문 탭의 variant 선택 아래에 엔진 모듈 카드 선택기를 추가했다. 카드 이름과 설명은 서버 카탈로그에서 내려오므로 화면 쪽에 이름이 중복 정의되지 않으며, 선택값은 대화 생성·전송·리롤·수정 네 경로의 요청에 모두 실린다.
- [문서] 활성 사용자 인터페이스를 `graphrag-chat-site/`로 바꾸고, 기존 두 클라이언트를 레거시로 정리했다.
- [기능] 산문 프롬프트를 A/B/C/D variant에서 프로필 구조로 바꿨다. 이제 `base` 하나와 `primary` 하나에 필요한 `modifiers`만 얹는 방식이고, 문체 지시가 나오는 자리는 이 한 축뿐이다. 기존 선택은 한 릴리스 동안 그대로 받아서 B와 C는 완화 보정으로, A와 D는 기본 프로필로 올린다. 저장된 대화를 읽을 때도 같은 승격이 걸린다.
- [기능] Fixed 세그먼트를 operator, 유저 소유권, 항상 적용되는 simulation core, 산문 프로필, 엔진 모듈, 저작된 정적 자산 순으로 다시 세웠다. pov·emotion·style·core·npc_behavior로 흩어져 있던 다섯 블록과 variant별 자산 선택은 사라졌다.
- [기능] Genre 세그먼트를 없앴다. 프롬프트는 Fixed와 Dynamic 두 개이고, scene type은 상태 라우팅과 블랙리스트 선택에만 쓰인다. 씬별 산문 Markdown과 few-shot 예시 조립도 함께 걷어냈다.
- [기능] 턴 체크리스트 템플릿을 없애고 그 안에 있던 상태를 Dynamic 블록으로 옮겼다. 현재 시점, 턴 단위 제약, 생식 상태가 각각 별도 블록으로 나가며, 생식 상태 블록은 성인 엔진이 켜져 있을 때만 렌더된다.
- [기능] Actor 출력 계약을 짧은 System_Log로 교체했다. `<analyze>` 안에서 Mode·Input·Cause·Cast·Talk·Delta·Guard를 한 줄씩 쓰고 관계 변화가 있을 때만 Rel을 더한다. 본문 첫 줄의 날짜·시간·장소 헤더 규약과 기존 시간 파서 호환은 그대로다.
- [기능] 등장 인물 목록을 Actor 분석문 파싱이 아니라 Manager의 활성 캐스트에서 확정하게 했다. 모델이 자기 분석문에 무엇을 적었는지와 무관하게 같은 값이 나온다.
- [기능] 메인 엔진 슬롯을 카탈로그에서 제거했다. 이 슬롯이 들고 있던 마크업·단락·리듬 지시가 산문 축과 정면으로 부딪혔기 때문이고, 함께 있던 메인·기억 정책 동기화 불변식도 같이 사라졌다. 남는 선택형 엔진은 여섯 개다.
- [기능] 성인 엔진에는 게이트·합의·페이즈·유저 통제·신체 연속성만 남기고 표현 규칙은 성인 modifier 네 개로 옮겼다. 거절과 중단 요청이 몸의 반응보다 우선한다는 규정, 피임 분기, 내사정 합의 요건은 엔진 쪽에 명시적으로 보존했다.
- [기능] 기억 엔진의 감정 직접 명명 금지와 이모지 금지, 인물기억 엔진의 종결 비트 지시, 다인플레이 엔진의 문장 길이 지시를 걷어냈다. 세 모듈 모두 자기 기계 규칙만 남는다.
- [API] 산문 카탈로그 경로를 `/api/prose-profiles`로 옮기고 응답 payload를 `proseProfile`로 바꿨다. 생성·전송·리롤·수정 요청은 `prose_profile`을 받고, 옛 `prose_variant` 문자열도 한 릴리스 동안 함께 받는다.
- [UI] 활성 UI의 산문 탭을 기반·주 문체 선택과 보정 토글 구조로 바꿨다. 로컬 저장 값은 새 구조로 저장되고, 옛 문자열이 남아 있으면 읽을 때 한 번 승격시킨다.
- [문서] 프롬프트 계약 문서와 Wiki 프롬프트 파이프라인 문서를 두 세그먼트 구조와 System_Log 계약으로 갱신했다.

## 2026-09-13

- [기능] 엔진 모듈 카탈로그에 메인 엔진 슬롯을 되살렸다. 문체 축과 충돌하던 마크업·단락·리듬 지시는 뺀 채로 슬롯만 복원했고, 메인·기억 엔진이 공유하던 사용자 서술 정책 동기화 불변식도 함께 되살렸다. 동기화는 카탈로그의 정책 그룹 값을 그대로 따라가는 방식이라 슬롯이 늘거나 바뀌어도 코드를 새로 손댈 필요가 없다.
- [기능] 성인 문체 보정 네 가지를 하나만 고를 수 있는 배타 그룹으로 묶었다. 여러 개를 고르면 가장 나중에 고른 것만 남고, 성인 엔진이 켜져 있을 때만 실제로 적용된다. 성인 엔진이 켜졌는데 아무 보정도 고르지 않으면 상업지 문법이 기본값으로 붙는다. 이 결합은 조립 시점에만 적용되고 대화에 저장된 선택값 자체는 그대로 유지된다.
- [기능] Dynamic 출력 계약이 엔진 모듈 블록을 본문 앞뒤 어디에 배치할지까지 소유하도록 범위를 넓혔다. 출력 전 침묵 검증 항목도 관점, 관측 가능 범위, 모드와 게이트, 인과 연속성, 유효한 인계 지점, 엔진 블록 배치를 포함하도록 확장했다.
- [버그픽스] 산문 기반 규약의 날짜·시간·장소 헤더 예시 표기를 바로잡고, 의성어·의태어 표기를 신체 접촉음으로만 한정하던 제약을 없앴다.
- [UI] 활성 UI의 산문 보정 토글이 같은 배타 그룹 안에서는 하나만 선택되게 강제하도록 고쳤다. 켜는 순간 같은 그룹의 다른 보정은 자동으로 꺼지고, 끄는 동작은 그대로다.
- [UI] 모델이 응답에 쓰는 엔진 모듈 `<details>` 블록을 분석·OOC 기록과 같은 방식으로 접었다 펼 수 있게 했다. 블록은 본문에서 떼어내 거터의 토글 버튼으로 열고, 날짜·시간·장소 헤더 앞에 블록이 와도 헤더가 장면 정보로 인식된다. 스트리밍 중 아직 닫히지 않은 블록은 본문에 태그째 드러나지 않는다.

## 2026-09-14

- [기능] 범위별·엔진 모드별로 나뉘어 있던 유저노트 저장소를 하나의 전역 라이브러리로 합쳤다. 이름과 본문은 이제 모든 대화와 Graph/Wiki 모드에서 공유되고, 켜고 끄는 상태만 대화 thread마다 독립적으로 저장된다. 기존에 흩어져 있던 노트와 스레드에 남아 있던 레거시 노트는 첫 접근 시 자동으로 전역 라이브러리에 합쳐지며, 각 대화가 이전에 실제로 보고 있던 켜짐 상태는 그대로 보존된다. 응답 스트리밍 중 저장되는 경합 방지 로직도 켜짐 상태 필드를 함께 다시 읽도록 넓혔다. API 응답 모양은 바뀌지 않아 클라이언트 쪽 변경은 없다.
- [버그픽스] Wiki vault 진단의 중복 id 검사가 런타임이 허용하는 문서 재정의 구조(상위 캐릭터 프로필이나 장면 문서를 하위 분기 디렉터리의 같은 id 문서로 덮는 구성)를 오류로 잘못 보고하던 문제를 고쳤다. 이제 상위 트리와 분기 디렉터리마다 id 공간을 따로 두고, 같은 id·같은 type일 때만 의도된 재정의로 인정한다. type이 다르면 여전히 오류이고, 분기 디렉터리끼리 같은 id를 쓰는 것은 허용한다.
- [버그픽스] 메시지 전송·리롤·수정·대화 생성 요청에 `prose_profile`을 빼고 보내면 대화에 저장된 산문 프로필이 기본값으로 덮어써지던 문제를 고쳤다. 라우터가 빈 요청에서도 기본 프로필을 만들어 넘기던 탓이다. 이제 요청 모델이 경계에서 프로필을 검증하고, 값이 없으면 저장된 선택을 그대로 쓴다. 옛 `prose_variant` 문자열은 계속 받고, 형식이 깨진 프로필은 422 대신 기본값으로 떨어진다.
- [리팩토링] 프롬프트 조립 경로에서 중복을 걷어냈다. 이름은 같고 동작이 다른 변수 치환 함수 두 개를 `renderers.py`의 명시적인 두 함수로 합쳤고, 엔진 모듈 블록을 치환에서 빼려고 쓰던 표시용 문자열 클래스를 없앴다. Fixed 세그먼트가 PromptBuilder가 이미 정규화한 프로필·엔진 선택을 다시 정규화하던 부분, 성인 엔진·통합 블랙리스트·인물 초점 판정의 중복, 쓰이지 않던 `build()`의 `recent_story` 인자와 체크리스트·레거시 유저노트 잔재 함수도 정리했다. 앱 계층에 여섯 군데 복사돼 있던 Actor 모델·산문 프로필·엔진 모듈 선택 해석은 `resolve_generation_selection` 하나로 모았다. 조립된 프롬프트는 245개 설정 조합에서 바이트 단위로 같다.
- [개선] Graph 모드가 Dynamic 프롬프트의 출력 계약 뒤에 따로 덧붙이던 `[Character Naming Lock]`과 `[Previous Turn CoT]` 블록을 없앴다. 출력 계약이 다시 Dynamic의 마지막 블록이 되고, 이전 턴 System_Log를 넘기려고 대화 상태·보류 커밋·리롤 롤백에 흩어져 있던 `prev_cot` 배관도 함께 걷어냈다. 예전에 저장된 대화나 보류 커밋 파일에 이 키가 남아 있어도 그대로 읽힌다. `build_prompt_parts`에서 쓰이지 않던 `recent_story` 인자도 뺐다.
