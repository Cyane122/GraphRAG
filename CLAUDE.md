...
`cp example.env .env` → 크리덴셜 채우기. 린트 없음. 스모크 테스트는 `tests/smoke_*.py` (standalone, `python tests/smoke_<name>.py` 로 실행; pytest 설정 없음).
...
| `ANTHROPIC_API_KEY` | Claude Actor 모델용 Anthropic direct API key (Vertex 비용/쿼터 소진 시 폴백) |
| `ANTHROPIC_CLAUDE_SONNET_MODEL` | Sonnet UI 옵션에 매핑할 다이렉트 Anthropic 모델 ID |
| `ANTHROPIC_CLAUDE_OPUS_4_6_MODEL` | Opus 4.6 UI 옵션에 매핑할 다이렉트 Anthropic 모델 ID |
| `ANTHROPIC_CLAUDE_OPUS_4_7_MODEL` | Opus 4.7 UI 옵션에 매핑할 다이렉트 Anthropic 모델 ID |
| `ANTHROPIC_CLAUDE_OPUS_4_8_MODEL` | Opus 4.8 UI 옵션에 매핑할 다이렉트 Anthropic 모델 ID |
| `ANTHROPIC_VERTEX_REGION` | Claude on Vertex 리전 (기본 `GOOGLE_CLOUD_LOCATION`=global; Claude 권장값) |
...
| 파트 | 내용 | 규칙 |
|------|------|------|
| **Fixed** | policy + world + char 고정 설명 | 턴 간 동일 → Gemini implicit cache hit / Claude는 `cache_control` breakpoint (actor.py `_claude_system_blocks`) |
