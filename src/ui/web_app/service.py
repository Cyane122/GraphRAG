...
from src.ui.social_media_settings import resolve_social_media_features
from src.ui.debug_graph import build_debug_graph
from src.ui.turn_debug import write_turn_debug_snapshot
from src.ui.web_app.actor import stream_actor_events
from src.ui.web_app.analysis_tools import render_database_tool
from src.ui.web_app.commit import commit_pending_web
from src.ui.web_app.models import ChatMessage, ConversationState, MessageVariant, normalize_actor_model
from src.ui.web_app.runtime import ActiveConversation, initialize_conversation, restore_game_time, snapshot_game_time
...
async def refresh_graph_snapshot_best_effort(state: ConversationState) -> None:
    ...
    try:
        graph = await build_debug_graph(pc_id=state.pc_id, npc_id=state.npc_id, world_id=state.world_id)
        graph.thread_id = state.thread_id
        ensure_graph_server()
        update_graph_snapshot(graph)
    except Exception as exc:
        print(f"[web_app] graph snapshot refresh skipped: {exc}")


async def _repair_if_needed(full_response: str, state: ConversationState) -> str:
    """Repair output guard violations before persistence."""
    blocked_terms = find_forbidden_terms(full_response) + find_pov_violations(full_response, state.perspective)
    if not blocked_terms:
        return full_response
    repaired = await repair_actor_output(full_response, blocked_terms, MODEL_OUTPUT_REPAIR)
    if find_forbidden_terms(repaired):
        raise RuntimeError("Actor output failed the output guard after repair.")
    return repaired
...
async def _run_generation_events(
    ...
    *,
    ...
    note_block = build_usernote_block(load_usernote(state.thread_id))
    if note_block:
    ...
    full_response = await _repair_if_needed(final_event["content"], state)
...
async def reroll_assistant(
    ...
    async with ActiveConversation(state):
        ...
        try:
            ...
            # 재생성 실패: rerolled 응답을 제거하고 history/recent를 messages에서 재구성해 일관된 '폐기'
            # 상태로 영속화한다. 폐기된 pending이 되살아나지도(resurrect), pending 없는 보이는 응답이
            # 남지도, messages↔history가 어긋나지도 않는다. (사용자 입력은 남아 재시도 가능.)
            state.messages = [msg for msg in state.messages if msg.id != assistant.id]
            state.history = [
...
async def edit_message(
    ...
    async with ActiveConversation(state):
        ...
        message.content = content
        message.edited = True
        paired = next((msg for msg in state.messages if msg.parent_user_id == message.id), None)
        if paired and state.pending_commit and state.pending_commit.get("response_msg_id") == paired.id:
