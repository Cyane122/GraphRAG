...
#   - load_pending_commit(world_id: str, pc_id: str, npc_id: str) -> dict | None : Load the latest uncommitted pending snapshot.
...
def _candidate_paths(world_id: str, pc_id: str, npc_id: str) -> list[Path]:
    ...
    if thread_id:
        prefix = "__".join(
            [_safe_part(world_id), _safe_part(pc_id), _safe_part(npc_id), _safe_part(thread_id)]
        )
...
def load_pending_commit(world_id: str, pc_id: str, npc_id: str) -> dict | None:
    """Load the latest uncommitted pending snapshot."""
    for path in _candidate_paths(world_id, pc_id, npc_id):
        try:
