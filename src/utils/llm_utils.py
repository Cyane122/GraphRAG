...
Gemini 모델 초기화(get_model)와 LLM 응답 JSON 파싱(extract_json_from_llm)을 제공.

get_model()은 _GeminiModel 래퍼를 반환한다.
agent 파일에서 model.generate_content() / model.generate_content_async() 형태로
호출하며, generation_config 딕셔너리 안의 thinking_config 키를
types.ThinkingConfig으로 자동 변환해 Gemini 3 API 규격에 맞춘다.

- generate_content: 동기 호출 (actor_agent 등)
- generate_content_async: 비동기 호출 (complex_updater, decay_manager 등) — client.aio 사용
- get_client(): app.py 스트리밍에서 직접 클라이언트 접근용
- get_response_text(): response.text가 None일 때 parts에서 텍스트 추출 (thinking 모드 대응)
- extract_json_from_llm(): None 입력 guard 추가
- aiplatform.init / vertexai.GenerativeModel 의존 제거
- genai.Client(vertexai=True) 단일 클라이언트로 통합
"""
