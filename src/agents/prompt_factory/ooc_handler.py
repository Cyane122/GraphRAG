...
Current in-game time: {current_time}
...
new_datetime — the new in-game datetime as "YYYY-MM-DDTHH:MM:00", or null
  Use {current_time} as the reference. Compute the exact new datetime.
  "2026년 5월 15일" → replace date, keep current hour: "2026-05-15THH:MM:00"
  "며칠 후" → add 3-5 days to current date, keep current hour
  "다음날 아침" → next date at T08:00:00
...
time_set — legacy fallback only ("HH:MM" or null). Use new_datetime above.

location_id
  - If the destination exists in Available Location IDs, use that exact ID.
  - Use the Current Location Chain to resolve short aliases:
    e.g. "급식실" while inside 성화고등학교 → use the existing child/sibling ID from Available Locations.
    Match by name, alias, or tag — do not invent a new ID if a match already exists.
  - If no existing location matches, create a stable lowercase snake_case ID.
  - Use null if characters do not clearly move to a different location node.
  - Kitchen/bathroom/bedroom within the current location = still the same location (null).

new_location
  - null when using an existing location or no location change.
  - object only when location_id is a new ID not listed in Available Location IDs.
  - shape: {"name": "display name", "aliases": ["short alias"], "description": "short factual description", "prompt_hint": "sensory/context hint", "parent_location_id": "existing_parent_id_or_null", "tags": ["dynamic"], "prompt_priority": 8}
  - parent_location_id: if the new place is inside or adjacent to the current location chain,
    use the innermost relevant ID from Current Location Chain. Otherwise null.

state_changes — JSON object mapping character name to their DynamicState updates.
  Only include a character if they are explicitly the SUBJECT of a physical or emotional state change.
  Omit characters who are merely observed, mentioned, or acting on others without themselves changing state.
  Allowed keys per character: mood, mental_condition, stress_level, physical_condition, injury_detail, emotional_state
  mood: calm/happy/sad/angry/anxious/tired/annoyed/excited
  emotional_state: short Korean phrase (e.g. "설렘", "불안", "안도", "긴장")
  stress_level: integer 0-10 ONLY (never strings like "high")
  physical_condition: healthy/fatigued/injured/ill/hospitalized
  injury_detail: body part + type (e.g. "허리 염좌", "발목 삠")
  Past-tense injury or fatigue descriptions = the character IS in that state NOW.

  Use the character name exactly as listed in Known Characters above.

  Examples:
  "*잘 자네.*" → {}
  "*박시안은 좀 화난 듯하다.*" → {"박시안": {"mood": "angry"}}
  "*유람이 뛰어왔다. 박시안은 걱정스럽다.*" → {"유람": {"physical_condition": "fatigued"}, "박시안": {"mood": "anxious"}}
  "*박시안은 허리를 삐끗했다.*" → {"박시안": {"physical_condition": "injured", "injury_detail": "허리 염좌"}}
  "*유람이 뛰어온다.*" → {"유람": {"physical_condition": "fatigued"}}
  "*유람이 박시안을 바라본다.*" → {} (동작만, 상태 변화 없음)
...
async def parse_ooc(text: str, npc_id: str, npc_name: str, pc_id: str | None = None) -> dict:
