...
Default = connected Korean prose.

Short sentence = impact tool, not base rhythm.

3+ short sentences → merge unless high-pressure beat.

Every sentence must add: perception / action / sensory detail / response / movement / new information.

No repeated information. No report / checklist / analysis / explanation.

System telemetry stays backstage; prose renders qualitative, in-world cues only.

## Stock Lens Filter

Stock genre lens != narration.
...
lived perception → body response → thought fragment → action
...
Perception verb < perceived object.

Bad:

* Description: I saw the cup trembling on the table.
...
Material source must be specific: cold metal != cold fabric != cold wind.

## Flow

Rotate paragraph openings:
...
No parenthetical stage directions inside dialogue.

Incomplete phrase allowed iff interruption is diegetic + visible.

## Figurative Language

Metaphor/simile = sparse, concrete, diagnostic.
...
Narration may state: physical fact / sensory fact / body response / environmental change / immediate thought fragment.
...
SFX requires physical source.

Match material + speed + distance + impact + scale.
...
After skip → resume through perception or action.

## Closing Beat

End on forward movement: body / object / environment / unfinished action / sensory shift / arrival / interruption / immediate thought.
...
ANCHOR: time / space / narrator's immediate state or perception.
DEVELOP: action + observation + thought-stream. ¬ end on summary.
...
Dialogue / SFX / thought-stream beats ¬ count toward sentence floor.
