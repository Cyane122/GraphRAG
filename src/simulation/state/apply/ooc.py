# ================================
# src/simulation/state/apply/ooc.py
#
# Load OOC state context and apply parsed OOC time, location, character, and global-state changes.
#
# Functions
#   - handle_ooc(text: str, npc_id: str, npc_name: str, pc_id: str | None = None, world_config: dict | None = None) -> dict : Parse and apply an OOC command through the simulation-owned coordinator
#   - build_ooc_parse_context() -> dict : Load persistent state required by the OOC parser
#   - apply_ooc_plan(text: str, npc_id: str, pc_id: str | None, parsed: dict) -> dict : Apply one parsed OOC plan to persistent state
# ================================
...
from src.core.database import async_driver, ensure_location, move_location, update_dynamic_state
...
async def build_ooc_parse_context() -> dict:
    """Load all persistent state required to classify one OOC command."""
    async with async_driver.session() as session:
        rec = await session.run("MATCH (gs:GlobalState {id: 'singleton'}) RETURN gs.currentTime AS ct")
        row = await rec.single()
        current_time = _parse_current_time(row["ct"] if row else None)
    try:
        schedule_context = await fetch_schedule_context(current_time=current_time, window_minutes=SCHEDULE_TIME_PARSE_WINDOW_MIN)
    except Exception as exc:
        print(f"[OOC] schedule context fetch failed (ignored): {exc}")
        schedule_context = {}
    return {"current_time": current_time, "schedule_context": schedule_context, "locations": await _get_allowed_locations(), "characters": await _get_all_characters(), "location_context": await _fetch_location_context(), "rule_hints": await _fetch_ooc_rule_hints()}


async def apply_ooc_plan(text: str, npc_id: str, pc_id: str | None, parsed: dict) -> dict:
    """Apply a parsed OOC plan using its original persistent-state context."""
    plan = parsed["plan"]
...
async def handle_ooc(text: str, npc_id: str, npc_name: str, pc_id: str | None = None, world_config: dict | None = None) -> dict:
    """Parse through prompt_factory and apply the result in the state layer."""
    from src.agents.prompt_factory.ooc_handler import parse_ooc
    parsed = await parse_ooc(text, npc_id=npc_id, npc_name=npc_name, pc_id=pc_id, world_config=world_config)
    return await apply_ooc_plan(text, npc_id, pc_id, parsed)
